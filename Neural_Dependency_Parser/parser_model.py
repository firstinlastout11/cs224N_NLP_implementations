...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # construct the bias term
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(1, hidden_size)))
        
        # construct the dropout layer
        self.dropout = nn.Dropout(p = dropout_prob)

        # constrcut self.hidden_to_logits
        self.hidden_to_logit_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(hidden_size, n_classes)))
        # construct the bias term
        self.hidden_to_logit_bias = nn.Parameter(nn.init.uniform_(torch.empty(1, n_classes)))
    ...
    def embedding_lookup(self, t):
        ...
        # Lopk up the embeddings for the input tokens in t
        x = self.pretrained_embeddings(t)

        # get the sizes of the x
        b_s, n_s, e_s = x.size()

        # reshape the x into (batch_size, n_features * embeddings_size)
        x = x.view(b_s, n_s * e_s)
        ### END YOUR CODE
        return x
    ...
    def forward(self, t):
        ...
        # Apply the look_up to t to get the mebeddings
        ...
        # Apply dropout lyayer to the hidden
        hidden = self.dropout(hidden)

        # Apply hidden to logits lyaer
        logits = torch.matmul(hidden, self.hidden_to_logit_weight) + self.hidden_to_logit_bias
