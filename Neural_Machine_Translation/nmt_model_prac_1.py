...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ###     self.h_projection (Linear Layer wse variables:
        ###     LSTM:ith no bias), called W_{h} in the PDF.
        ...
        ###
        ### Use the following docs to properly initialize the
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # construct a tensor X of shape (src_len, b, e)
        
        #X = torch.zeros(max(source_lengths), len(source_lengths), self.embed_size)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # apply the attention projection layer
        ...
        for Y_t in torch.split(Y, 1, dim = 0):
            ...
            # to compute the input vector for the decoder at timestep t, we concat the original input for decoder (Y_t) and the combined output vector of the previous timestep
            # embedding_size + hidden_size
            Ybar_t = torch.cat((Y_t, o_prev), dim = 1)

            # Use the step function compute the decoder's next (cell, state) values
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Concatenate the dec_hidden w/ a_t to compute U_t
        U_t = torch.cat((dec_hidden, a_t), dim = 1)

        # apply the combined output projection to compute V_t
        V_t = self.combined_output_projection(U_t)

        # compute tensor O_t by first applying Tanh and dropout
