...
class ModelEmbeddings(nn.Module):
    """
    Class that converts input words into embeddings
    """
    ...
    def __init__(self, embed_size, vocab):
        ...
        # Embedding for the pad
        src_pad_token_idx = vocab.src['<pad>']
        tgt_pad_token_idx = vocab.tgt['<pad>']

        # Initialize the Embedding layer for source language
        self.source = nn.Embedding(len(vocab.src), self.embed_size, padding_idx = src_pad_token_idx)

        # Initiazlie the Embedding layer for target language
