...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTM(input_size = self.embed_size + self.hidden_siz,
                                hidden_size = self.hidden_size,
                                bias = True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        o_prev = torch.zeros(batch_size, self_hidden_size, device = self.device)
        ...
        for Y_t in torchsplit(Y, 1, dim = 1):
            ...
            # We only use the decoder's output vctor as an input to the next cell
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.squeezee(torch.bmm(torch.unsqueeze(alpha_t, dim_1), enc_hiddens), dim = 1)
