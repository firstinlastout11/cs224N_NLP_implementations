...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        This method should conver characters in the input sentences into their corresponding character indices using
        the character vocabulary char2id defiend above

        """

        # Initialize the list to store all the indices
        word_ids = []
        # Iterate through each sentence
        for sent in sents:
            # Initiazlie a list to store all the words in a given sentence
            sent_lst = []
            # Iterate thru each word in a sentence
            for word in sent:
                # Initialize a list to store the indices for all characters in the given word. 
                # The first index is for the start of the word 
                char_lst = [self.start_of_word]

                # Iterate thru each char in the given word
                for c in word:
                    # If the given charcter is in the character-vocabulary
                    if c in self.char2id:
                        char_lst.append(self.char2id[c])

                    # If the given character is Out-of-vocabulary
                    else:
                        char_lst.append(self.char_unk)
                # Append the end_of_word index at the end of each word
                char_lst.append(self.end_of_word)

                # Append the indices of chars in a given word to the sentnece_lst
                sent_lst.append(char_lst)

            # Append the list of words in a given sentende into the whole list
            word_ids.append(sent_lst)
