...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # Keep this for predictions
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)

        # Initialize soure_padded_chars for character level padded encoding for source
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device = self.device)

        # Initialize target_padded_chars for character level padded encoding for target
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device = self.device)

        # Apply the encoder to `source_padded_chars` by calling `self.encode()`
        enc_hiddens, dec_init_state = self.encode(source_padded_chars, source_lengths = source_lengths)
